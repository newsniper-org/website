...
function render_folded_block(block)
    ...
    if not (quarto._quarto.format.isHtmlOutput() or quarto._quarto.format.isMarkdownWithHtmlOutput()) then
  ...
  function code_block(code_block_slot, filename)
    function process(el)
      ...
      if lang or title or showLineNumbers then
        ...
        -- astro code block attributes don't conform to any syntax
    ...
    if code_block_slot.t == "CodeBlock" then
      ...
      return quarto._quarto.ast.walk(code_block_slot, {
        CodeBlock = process
      })
    end
  end
